...
let encoded = try? PropertyListEncoder().encode(arr1)

let decoded = try? PropertyListDecoder().decode([MacBook].self, from: encoded!)

print(decoded)

let dict1 = ["1": macBook, "2": macBook]

let encoded1 = try? PropertyListEncoder().encode(dict1)

let decoded1 = try? PropertyListDecoder().decode([String: MacBook].self, from: encoded1!)
